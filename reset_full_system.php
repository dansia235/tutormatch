<?php
/**
 * Script d'installation complète du système de tutorat
 * 1 admin, 52 tuteurs, 205 étudiants, 315 stages
 * Affectations cohérentes par département
 */

echo "<!DOCTYPE html><html><head><title>Installation Système Tutorat</title></head><body>";
echo "<h1>🎓 Installation Complète - Système de Tutorat</h1>";
...
try {
    ...
    // Générer tous les noms d'étudiants avec indices uniques
    $studentNames = [];
    for ($i = 0; $i < 205; $i++) {
        ...
        if ($i >= count($studentFirstNames)) {
            $firstName .= ($i - count($studentFirstNames) + 1);
        }
        if ($i >= count($studentLastNames)) {
            $lastName .= ($i - count($studentLastNames) + 1);
        }
    ...
    $internshipTitlesByDept = [
        'Informatique' => [
            'Développeur Web Frontend', 'Développeur Web Backend', 'Développeur Full Stack',
            'Data Scientist Junior', 'Analyste de Données', 'Ingénieur Big Data',
            'Développeur Mobile iOS', 'Développeur Mobile Android', 'Développeur React Native',
            'Ingénieur DevOps', 'Administrateur Systèmes', 'Architecte Cloud',
            'Spécialiste Cybersécurité', 'Auditeur Sécurité IT', 'Consultant IT',
            'Développeur Intelligence Artificielle', 'Ingénieur Machine Learning', 'UX/UI Designer'
        ],
        'Génie Civil' => [
            'Ingénieur Bureau d\'Études', 'Chef de Chantier Junior', 'Conducteur de Travaux',
            'Ingénieur Structure', 'Chargé d\'Affaires BTP', 'Technicien Géotechnique',
            'Ingénieur Environnement', 'Urbaniste Junior', 'Dessinateur-Projeteur',
            'Ingénieur Travaux Publics', 'Responsable QSE', 'Métreur-Vérificateur'
        ],
        'Électronique' => [
            'Ingénieur Systèmes Embarqués', 'Développeur IoT', 'Technicien Électronique',
            'Ingénieur Télécommunications', 'Automaticien', 'Ingénieur Signal',
            'Technicien Réseau', 'Ingénieur Test Électronique', 'Concepteur Circuits',
            'Ingénieur Radiofrequence', 'Spécialiste FPGA', 'Ingénieur Microélectronique'
        ],
        'Mécanique' => [
            'Ingénieur Conception Mécanique', 'Dessinateur Industriel', 'Technicien Maintenance',
            'Ingénieur Production', 'Roboticien', 'Ingénieur Qualité',
            'Technicien Usinage', 'Ingénieur Procédés', 'Chargé d\'Industrialisation',
            'Ingénieur R&D Mécanique', 'Technicien Métrologie', 'Pilote d\'Îlot de Production'
        ],
        'Mathématiques' => [
            'Analyste Quantitatif', 'Data Analyst', 'Statisticien',
            'Consultant Actuariel', 'Modélisateur Financier', 'Chercheur en Mathématiques Appliquées'
        ]
    ];
    ...
    foreach ($departments as $dept => $config) {
        ...
        $titles = $internshipTitlesByDept[$dept];
        ...
        for ($i = 0; $i < $targetCount; $i++) {
            ...
            if ($i >= count($titles)) {
                $title .= ' #' . (intval($i / count($titles)) + 1);
            }
            
            $companyId = $companies[rand(0, count($companies) - 1)];
            
            // Dates variables
            $startDates = ['2024-07-01', '2024-08-01', '2024-09-01', '2024-10-01'];
            $startDate = $startDates[rand(0, count($startDates) - 1)];
            $endDate = date('Y-m-d', strtotime($startDate . ' +3 months'));
            
            $description = "Stage en $dept - $title. Mission enrichissante dans une équipe dynamique avec possibilité d'évolution.";
            $requirements = "Étudiant en $dept, niveau " . $levels[rand(2, 4)] . ". Motivation et esprit d'équipe requis.";
            
            $stmt = $db->prepare("INSERT INTO internships (title, company_id, start_date, end_date, description, status) VALUES (?, ?, ?, ?, ?, 'available')");
            $stmt->execute([$title, $companyId, $startDate, $endDate, $description]);
        ...
        logProgress("✅ {$targetCount} stages créés pour $dept");
    }
    ...
    try {
        ...
        foreach ($allAssignments as $assignment) {
            ...
            if (in_array('assignment_id', $evalColumns)) {
                $stmt = $db->prepare("INSERT INTO evaluations (assignment_id, student_id, teacher_id, type, score, comments) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$assignment['assignment_id'], $assignment['student_id'], $assignment['teacher_id'], 'mid_term', $midTermScore, $midTermComments[rand(0, count($midTermComments) - 1)]]);
            } else {
                $stmt = $db->prepare("INSERT INTO evaluations (student_id, teacher_id, type, score, comments) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$assignment['student_id'], $assignment['teacher_id'], 'mid_term', $midTermScore, $midTermComments[rand(0, count($midTermComments) - 1)]]);
            }
            $evaluationCount++;
            ...
            if (in_array('assignment_id', $evalColumns)) {
                $stmt = $db->prepare("INSERT INTO evaluations (assignment_id, student_id, teacher_id, type, score, comments) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$assignment['assignment_id'], $assignment['student_id'], $assignment['teacher_id'], 'final', $finalScore, $finalComments[rand(0, count($finalComments) - 1)]]);
            } else {
                $stmt = $db->prepare("INSERT INTO evaluations (student_id, teacher_id, type, score, comments) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$assignment['student_id'], $assignment['teacher_id'], 'final', $finalScore, $finalComments[rand(0, count($finalComments) - 1)]]);
            }
            $evaluationCount++;
    ...
    echo "<div style='color: red; background: #ffe6e6; padding: 20px; border-radius: 10px; border-left: 5px solid #dc3545;'>";
    echo "<h3 style='color: #dc3545; margin-top: 0;'>❌ Erreur durant l'installation</h3>";
    echo "<p><strong>Message:</strong> " . $e->getMessage() . "</p>";
    echo "<p><strong>Ligne:</strong> " . $e->getLine() . "</p>";
    echo "<p><strong>Fichier:</strong> " . $e->getFile() . "</p>";
    echo "<div style='background: rgba(0,0,0,0.1); padding: 10px; border-radius: 5px; margin-top: 10px; font-family: monospace; font-size: 0.9em;'>";
    echo "<strong>Trace:</strong><br>" . nl2br($e->getTraceAsString());
    echo "</div>";
    echo "</div>";
}

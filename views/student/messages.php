<?php
/**
 * Vue pour la messagerie de l'étudiant
 * Permet d'afficher et d'envoyer des messages liés au stage
 */
...
// Récupérer l'ID utilisateur de l'étudiant (pas l'ID de la table students)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    if ($receiverId && !empty($content)) {
        ...
        if ($messageId) {
            ...
            redirect('/tutoring/views/student/messages.php');
        } else {
...
// Si l'étudiant a une affectation active, récupérer son tuteur
if ($activeAssignment) {
    // Récupérer le tuteur académique
    ...
    if ($teacher) {
        ...
        if ($teacherUser) {
            $contacts[] = [
                'id' => $teacherUser['id'], // Utiliser l'ID utilisateur
...
// Ajouter les coordinateurs et administrateurs

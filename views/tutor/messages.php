<?php
/**
 * Vue pour la messagerie des tuteurs
 * Permet d'afficher et d'envoyer des messages
 * Basée sur la structure de messages.php de l'étudiant
 */
    ...
    if (isset($_POST['send_message'])) {
        ...
        try {
            ...
            if ($messageId) {
                ...
                redirect('/tutoring/views/tutor/messages.php');
            } else {
        ...
        error_log("Tentative de suppression de conversation. POST: " . json_encode($_POST));
        
        // Vérifier si l'ID est présent
        if (!isset($_POST['conversation_id']) || empty($_POST['conversation_id'])) {
            error_log("ID de conversation manquant dans: " . json_encode($_POST));
            setFlashMessage('error', 'ID de conversation manquant');
            redirect('/tutoring/views/tutor/messages.php');
            exit;
        }
        
        $conversationId = $_POST['conversation_id'];
        error_log("ID de conversation trouvé: " . $conversationId);
        
        try {
            // Vérifier si le format est conv_X_Y
            ...
            if (count($parts) >= 3 && $parts[0] === 'conv') {
                ...
                if ($user1 == $_SESSION['user_id'] || $user2 == $_SESSION['user_id']) {
                    // Marquer les messages comme supprimés pour cet utilisateur
                    $stmt = $db->prepare("UPDATE messages 
                                         SET status = 
                                            CASE 
                                                WHEN sender_id = :user_id AND status = 'sent' THEN 'sender_deleted' 
                                                WHEN receiver_id = :user_id AND status = 'sent' THEN 'receiver_deleted'
                                                ELSE status 
                                            END
                                         WHERE (
                                            (sender_id = :user1_id AND receiver_id = :user2_id) OR
                                            (sender_id = :user2_id2 AND receiver_id = :user1_id2)
                                         ) AND (sender_id = :user_id2 OR receiver_id = :user_id3)");
                    $stmt->bindParam(':user_id', $_SESSION['user_id']);
                    $stmt->bindParam(':user_id2', $_SESSION['user_id']);
                    $stmt->bindParam(':user_id3', $_SESSION['user_id']);
                    $stmt->bindParam(':user1_id', $user1);
                    $stmt->bindParam(':user1_id2', $user1);
                    $stmt->bindParam(':user2_id', $user2);
                    $stmt->bindParam(':user2_id2', $user2);
                    $success = $stmt->execute();
                } else {
                    $success = false;
                    error_log("L'utilisateur n'a pas accès à cette conversation");
                }
            } else if (is_numeric($conversationId)) {
                ...
                $success = false;
                error_log("Format d'ID de conversation non reconnu");
            }
            
            if ($success) {
                setFlashMessage('success', 'Conversation supprimée avec succès');
            } else {
                setFlashMessage('error', 'Erreur lors de la suppression de la conversation');
            }
...
foreach ($assignments as $assignment) {
    $contacts[] = [
        'id' => $assignment['student_user_id'], // Utiliser l'user_id de l'étudiant
        'type' => 'student',
...
// Ajouter les administrateurs à la liste des contacts
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form action="/tutoring/views/tutor/messages.php" method="POST" id="deleteMessageForm" style="display:none">
                    <input type="hidden" name="conversation_id" id="conversation_id_to_delete">
                    <button type="submit" name="delete_conversation" id="hidden-delete-button">Supprimer</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Gérer la suppression de conversation
        ...
        if (confirmDeleteButton) {
            confirmDeleteButton.addEventListener('click', function() {
                ...
                // Supprimer manuellement les éléments modaux
                ...
                // Soumettre le formulaire après fermeture du modal
                setTimeout(function() {
